...
class LauncherThemeFilesTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        code = r'''
typedef __SIZE_TYPE__ size_t;typedef int FRESULT;typedef struct {unsigned fsize;}FILINFO;
#define NULL ((void*)0)
...
static int status,calls;static char recorded[512];
int f_stat_char(const char*path,FILINFO*info){unsigned i=0;do{recorded[i]=path[i];}while(path[i++]);calls++;info->fsize=123;return status;}
'''
...
__declspec(dllexport) int locate(const char*device,const char*leaf,unsigned capacity,int error){
 static char out[512];FILINFO info;calls=0;status=error;recorded[0]=0;
